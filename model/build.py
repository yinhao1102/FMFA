...
class IRRA(nn.Module):
    ...
    def forward(self, image, text, ori_text):
        ...
        if 'efa' in self.current_task:
            ...
            if torch.isnan(t2isim).any() or torch.isinf(t2isim).any():
                print("Detected NaN or Inf in t2isim")

            agg_v = torch.matmul(t2isim, image_feats[:,1:,:])
            ret.update({'efa_loss':objectives.compute_minmax_info_loss(text_feats, image_feats[:,1:,:], agg_v)*self.args.minmax_loss_weight})
